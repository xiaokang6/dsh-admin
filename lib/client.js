  ...
  factory: (require) => {
    ...
    if (
      ...
      tag.textContent =
        ...
        '.dsa_btnUpdate{background:var(--dsw-alias-brand-primary,#4f8cff);color:#fff;border:1px solid var(--dsw-alias-brand-primary,#4f8cff);border-radius:6px;padding:4px 14px;font-size:12px;font-weight:600;cursor:pointer}' +
        '.dsa_btnUpdate:hover{filter:brightness(1.1)}' +
        '.dsa_btnUpdate:disabled{opacity:.5;cursor:default}' +
    ...
    const ZH = {
      ...
      'panel.update': '更新到最新版',
      'panel.updating': '正在更新…（npm install 可能需要几分钟）',
      'msg.updateDone': '更新完成，正在重启服务使新版本生效…',
      'msg.updateFailed': '更新失败：',
      'confirm.update': '将执行 npm install -g @deepseek-ai/dsh@latest 升级主程序，完成后自动重启（页面会短暂断开）。确定继续？',
      'panel.upToDate': '已是最新版本',
    };
    const EN = {
      ...
      'panel.update': 'Update to latest',
      'panel.updating': 'Updating… (npm install may take a few minutes)',
      'msg.updateDone': 'Update complete; restarting the service…',
      'msg.updateFailed': 'Update failed: ',
      'confirm.update': 'Run npm install -g @deepseek-ai/dsh@latest and restart (page will disconnect briefly)?',
      'panel.upToDate': 'Up to date',
    };
    ...
    function AdminHeaderButton() {
      ...
      const doUpdate = () => {
        if (typeof window === 'undefined') return;
        if (!window.confirm(t('confirm.update'))) return;
        setMsg({ ok: true, text: t('panel.updating') });
        postJson(BASE + '/update')
          .then((r) => {
            if (!r || !r.ok) throw new Error((r && (r.error || r.msg)) || 'update failed');
            setMsg({ ok: true, text: r.message || t('msg.updateDone') });
            // 更新成功后 host 会自动调度重启；页面会在重启后恢复
          })
          .catch((e) => setMsg({ ok: false, text: t('msg.updateFailed') + String(e && e.message ? e.message : e) }));
      };

      const b = badgeOf(data, loading, error, t);
      const el = react.createElement;
      const v = data && data.current ? data.current : '';
      const hasUpdate = !!(data && data.isNewer);
              ...
              el('div', { className: 'dsa_body' },
                el('div', { className: 'dsa_box' },
                  ...
                  el('div', { className: 'dsa_actions' },
                    ...
                    hasUpdate
                      ? el('button', { type: 'button', className: 'dsa_btnUpdate', onClick: doUpdate }, t('panel.update'))
                      : null)),
                el('div', { className: 'dsa_box' },
                  el('div', { className: 'dsa_row' },
